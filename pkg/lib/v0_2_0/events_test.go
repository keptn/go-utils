...
func TestKeptn_SendCloudEventWithRetry(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// 4 spans, 2 for each try
			// 1 - manual span inside obs service
			// 2 - span generated by the OTel http auto-instr library
			assert.Len(t, spans, 4)

			// the first try spans should have been recorded as error
			assert.Equal(t, codes.Error, spans[0].StatusCode())
			assert.Equal(t, codes.Error, spans[1].StatusCode())

			// the spans for the retry should be OK
			assert.Equal(t, codes.Unset, spans[2].StatusCode())
			assert.Equal(t, codes.Unset, spans[3].StatusCode())
		})
	}
}
