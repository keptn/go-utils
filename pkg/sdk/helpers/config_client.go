...
type SLIAndSLOReaderInterface interface {
	...
	GetSLOs(ctx context.Context, options GetSLOOptions) (*keptn.ServiceLevelObjectives, error)
}
...
type SLIAndSLOWriterInterface interface {
	...
	UploadSLIs(ctx context.Context, options UploadSLIOptions) error

	// UploadSLOs uploads the SLOs for the specified project, stage and service.
	UploadSLOs(ctx context.Context, options UploadSLOOptions) error
}
...
// GetSLOs gets the SLOs stored for exactly the specified project, stage and service.
func (rc *ConfigClient) GetSLOs(ctx context.Context, options GetSLOOptions) (*keptn.ServiceLevelObjectives, error) {
	resource, err := rc.client.GetServiceResource(ctx, options.Project, options.Stage, options.Service, options.SLOFileName)
	if err != nil {
		return nil, err
	}
...
// UploadSLOs uploads the SLOs for the specified project, stage and service.
func (rc *ConfigClient) UploadSLOs(ctx context.Context, options UploadSLOOptions) error {
	yamlAsByteArray, err := yaml.Marshal(options.SLOs)
	if err != nil {
		return fmt.Errorf("could not convert SLOs to YAML: %s", err)
	}

	return rc.client.UploadResource(ctx, yamlAsByteArray, options.SLOFileName, options.Project, options.Stage, options.Service)
}

// UploadSLIs uploads the SLIs for the specified project, stage and service.
func (rc *ConfigClient) UploadSLIs(ctx context.Context, options UploadSLIOptions) error {
	yamlAsByteArray, err := yaml.Marshal(options.SLIs)
	if err != nil {
		return fmt.Errorf("could not convert SLIs to YAML: %s", err)
	}

	return rc.client.UploadResource(ctx, yamlAsByteArray, options.SLIFileName, options.Project, options.Stage, options.Service)
}
...
func (rc *ConfigClient) GetShipyard(ctx context.Context, project string) (*keptnv2.Shipyard, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("unable to unmarshal shipyard content: %v", err)
	}
...
// GetSLIs gets the SLIs stored for the specified project, stage and service.
// First, the configuration of project-level is retrieved, which is then overridden by configuration on stage level, and then overridden by configuration on service level.
func (rc *ConfigClient) GetSLIs(ctx context.Context, options GetSLIOptions) (map[string]string, error) {
	slis := make(sliMap)
